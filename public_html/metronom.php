...
$month = get_request ( 'month' , '201311' ) ;
...
<tr><th>Wiki</th><td><input class='span2' type='text' name='language' value='$language' />.wikipediia</td></tr>
<tr><th>Month</th><td><input type='text' class='span2' name='month' value='$month' /> (YYYYMM)</td></tr>
...
if ( $user != '' && isset ( $_REQUEST['doit'] ) ) {
	...
	$sql = "select page_title,rev_timestamp FROM page,revision_userindex where page_id=rev_page and page_namespace=0 and rev_user_text='" . $db->real_escape_string($user) . "' and rev_parent_id=0" ;
	...
	foreach ( $pages AS $k => $page ) {
		if ( $k > 0 and $k % $blocksize == 0 ) {
			print "<p>$k of " . count($pages) . " articles checked...</p>" ;
			myflush() ;
//			break ;
		}
		...
		$url = "http://stats.grok.se/json/$wiki/$month/$page" ;
/*		
		$j = json_encode ( array ( $page ) ) ;
		$url = "http://tools-webserver-01/glamtools/viewstats_api.php?action=request&date=$month&pages=$j" ;*/
//		print "<pre>$url</pre>" ;
...
//		myflush();
		...
		$views[$page] = 0 ;
		if ( !isset ( $d->daily_views ) ) continue ;
		
		foreach ( $d->daily_views AS $a => $b ) $views[$page] += $b ;
	...
	$total = 0 ;
	print "<table class='table-condensed table-striped'><thead><tr><th>Page</th><th>Created on</th><th>Views in $month</th></tr></thead><tbody>" ;
