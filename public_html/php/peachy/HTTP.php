...
class HTTP {
	...
	function __construct( $echo = false ) {
		...
		curl_setopt($this->curl_instance,CURLOPT_MAXCONNECTS,500);
		...
		curl_setopt($this->curl_instance,CURLOPT_TIMEOUT,300);
		curl_setopt($this->curl_instance,CURLOPT_CONNECTTIMEOUT,100);
	...
	function post( $url, $data ) {
		...
		if( curl_errno( $this->curl_instance ) != 0 ) {
			print "<div>CURL error : " . curl_errno( $this->curl_instance ) . " / " . curl_error( $this->curl_instance ) . "</div>" ;
			throw new CURLError( curl_errno( $this->curl_instance ), curl_error( $this->curl_instance ) );
			return false;
		}
