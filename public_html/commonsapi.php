...
$api_version = '0.92' ; // Version
$api_data_page_url = 'http://commons.wikimedia.org/w/index.php?title=Commons:Commons_API&action=raw' ; // URL of license data page
...
function end_run ( $text ) {
  global $api_version , $imagelist ;
  print "<error>$text</error>" ;
  if ( count ( $imagelist ) > 1 ) print "</image>" ;
}
...
#  header('Content-type: text/plain; charset=utf-8');
  ...
  foreach ( $t2 AS $k => $v ) {
	...
	foreach ( $titles AS $k => $v ) {
		$v = trim ( implode ( "\n" , $v ) ) ;
//		$v = preg_replace ( '|^<p>(.+)</p>$|i' , '$1' , $v ) ;
		if ( substr ( $v , 0 , 3 ) == '<p>' && substr ( $v , -4 , 4 ) == '</p>' ) $v = substr ( $v , 3 , -4 ) ;
		$titles[$k] = $v ;
	}
#  exit;
  return $ret ;
...
foreach ( $imagelist AS $img ) {
	...
	if ( substr ( strtolower ( $img ) , 0 , 5 ) == 'file:' ) $img = substr ( $img , 5 ) ;

	if ( count ( $imagelist ) > 1 ) print '<image name="' . myurlencode(ucfirst($img)) . '">' ;
	...
	$ii_url = "http://commons.wikimedia.org/w/api.php?format=php&action=query&prop=imageinfo&iilimit=500&iiprop=timestamp|user|url|size|sha1|metadata&titles=File:" . $img ;
	...
	if ( !isset($data) ) {
		$ii_url = "http://commons.wikimedia.org/w/api.php?format=php&action=query&prop=imageinfo&iilimit=500&iiprop=timestamp|user|url|size|sha1|metadata&titles=File:" . urlencode ( $img ) ;
		if ( $thumb_width != 0 ) $ii_url .= '&iiurlwidth=' . $thumb_width ;
		if ( $thumb_height != 0 ) $ii_url .= '&iiurlheight=' . $thumb_height ;
	...
	$filedata = array () ;
	if ( !isset ( $data ) ) {
	  end_run ( 'File does not exist : ' . htmlentities ( $img ) ) ;
	  continue ;
	}
	foreach ( $data AS $k => $v ) {
	  $fd = $v ;
	...
	$found = false ;
	//if ( !$found ) 
	//$found = try_information_template ( $text ) ;
	//if ( !$found ) 
	$found = try_fileinfo_template ( $text ) ;
	...
	print '<title>' . myurlencode ( "File:$img" ) . '</title>' ;
	print '<urls>' ;
